...
def send_file(b,t,s,tempo,lp,loops_in):
   #loop iterations
    z=int(loops_in)
    #wp.wiringPiSetup()
...
submit_button = [sg.Button('Save', button_color = 'blue', enable_events=True),sg.Button("Submit",button_color = 'blue', enable_events=True)]
...
clear_all = sg.Button('Clear', button_color = 'red', enable_events = True, key = 'clearall')
right_column_group = [input_rows,clear_all,submit_button]
Inputs = [[sg.Text("Upload File"),sg.Input(key='midifilein',change_submits=True), sg.FileBrowse(), sg.Button('Convert', enable_events=True)],[sg.Text("Tempo:"), sg.Spin(list(range(1,121)),initial_value=60,key="tempo_input", size=5)],[sg.Button(button_text="Loop",button_color='black on gray',enable_events=True)]]
...
while True:
    ...
    if event == "Submit":
        ...
        submit_file(pressed,count,name,values['tempo_input'],loop, values['loops_in'] or 1000)
    if event == 'Convert':
