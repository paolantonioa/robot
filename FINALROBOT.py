# -*- coding: utf-8 -*-
"""
Spyder Editor

This is a temporary script file.
""" 
from midiutil import MIDIFile
import PySimpleGUI as sg
#import wiringpi as wp
import time
from threading import Timer
import serial
import threading as th
import mido as MD
#def update_file(step):
  #  print ("updated")
  #  global count 
  #  count = int(step)+1
ser = serial.Serial(port = '/dev/ttyS0',baudrate = 115200)
  
  #raspberry pi output code
#ser = serial.Serial(port='/dev/ttyS', baudrate = 115200)


def send_file(b,t,s,tempo,lp,loops_in, status):
   #loop iterations
    z=int(loops_in)
    #wp.wiringPiSetup()
    #serial = wp.serialOpen('/dev/ttyAMA0',9600)
    if lp ==1:
        while z > 0:
            
            for i in range (0,count-1): 
                
                ser.write(str.encode(str(b[i]) + str(t[i]) + str(s[i])))
                print(str(b[i]) + str(t[i]) + str(s[i]) )
                time.sleep((60/int(tempo)))
            z -= 1
        ser.write(str.encode(str(0) + str(0) + str(0)))   
        #exit()      
    else:
        
        for i in range (0,count-1):   
            #wp.serialPuts(serial, str(b[i]) + str(t[i]) + str(s[i]))
            print((str(b[i]) + str(t[i]) + str(s[i])) )
            ser.write(str.encode(str(b[i]) + str(t[i]) + str(s[i])))
            time.sleep(60/int(tempo))
        #exit()
        
    window[status].update(button_color = 'blue') 
    

def submit_file(pr,ct,nm,temp,loop_setting,loops_in, status):
    bass =[0]*16
    tone =[0]*16
    slap = [0]*16
    for i in range (0,len(pressed)):
        if int(pr[i][0]) < ct:
            bass[int(pr[i][0])-1] = pr[i][1]
        elif int(pressed[i][0]) < (ct*2)-1 and int(pressed[i][0]) > ct-1:
            tone[int(pr[i][0])-17] = pr[i][1]
        elif int(pressed[i][0]) > ct*2-2 and int(pressed[i][0])<ct*3-2:
            slap[int(pr[i][0])-33] = pr[i][1]
    print(bass,tone,slap)
    window[status].update(button_color = 'gray')

    send_file(bass,tone,slap,temp or 60,loop_setting, loops_in, status)
  
    

#for midifile
def submit_file2(pr,ct,nm,temp):
    print ("submitted")
    #degrees  = [60, 62, 64] # MIDI note number
    track    = 0
    channel  = 0
    time     = 0   # In beats
    duration = 1   # In beats
    tempo    = int(temp)  # In BPM
    volume   = 100 # 0-127, as per the MIDI standard

    MyMIDI = MIDIFile(1) # One track, defaults to format 1 (tempo track
                     # automatically created)
    MyMIDI.addTempo(track,time, tempo)
    print(len(pressed))
    for i in range (0,len(pressed)):
        if int(pressed[i][0]) < ct:
            MyMIDI.addNote(track, channel, 60, int(pressed[i][0])-1, duration, int(pressed[i][1])*40)
            
        elif int(pressed[i][0]) < (ct*2)-1 and int(pressed[i][0]) > ct-1:
            MyMIDI.addNote(track, channel, 62, int(pressed[i][0])-ct, duration, int(pressed[i][1])*40)
            
        elif int(pressed[i][0]) > ct*2-2 and int(pressed[i][0])<ct*3-2:
            MyMIDI.addNote(track, channel, 64, int(pressed[i][0])-(ct*2), duration, int(pressed[i][1])*40)

        else:
            print (pressed[i])
    for i in range (1,17):
        if str(pressed).find("('"+str(i)+"'") ==-1 and str(pressed).find("('"+str(i+16)+"'") ==-1 and str(pressed).find("('"+str(i+32)+"'") ==-1: 
            MyMIDI.addNote(track, channel, 58, i-1, duration, 1)
    with open("/home/pi/SDX/MIDI_Files/"+ nm +".midi", 'wb') as outf:
        MyMIDI.writeFile(outf)  
def upload_file(midi_in):
    pressed.clear()
    m = open(midi_in,'rb')
    midiin = MD.parse_all(m.read())
    count=1
   
    for i in range (3,len(midiin)):
        print(midiin[i])
        
        if str(midiin[i]).find('60')>0 and str(midiin[i]).find('on')>0:
            if str(midiin[i]).find('40')>0:
                pressed.append((str(count), 1))
            elif str(midiin[i]).find('80')>0:
                pressed.append((str(count), 2))
            elif str(midiin[i]).find('120')>0:
                pressed.append((str(count), 3))
            count=count+1
        elif str(midiin[i]).find('62')>0 and str(midiin[i]).find('on')>0:
            if str(midiin[i]).find('40')>0:
                pressed.append((str(count+16), 1))
            elif str(midiin[i]).find('80')>0:
                pressed.append((str(count+16), 2))
            elif str(midiin[i]).find('120')>0:
                pressed.append((str(count+16), 3))
            count=count+1 
        elif str(midiin[i]).find('64')>0 and str(midiin[i]).find('on')>0:
            if str(midiin[i]).find('40')>0:
                pressed.append((str(count+32), 1))
            elif str(midiin[i]).find('80')>0:
                pressed.append((str(count+32), 2))
            elif str(midiin[i]).find('120')>0:
                pressed.append((str(count+32), 3))
            count=count+1     
            
        
            
         
    for i in pressed:
        if int(i[0]) < 49:
            pressed_button(str(i[0]),int(i[1]))
    """
    #print(midiin.read())
    #mid = MD('test2.midi', clip = True)
       """ 
    
    print(midiin)
    
def p1(pressed):
    
    for i in pressed:
        if int(i[0]) < 49:
            pressed_button(str(i[0]),int(i[1]))

def p2(pressed):
    for i in pressed:
        if int(i[0]) < 49:
            pressed_button(str(i[0]),int(i[1]))
            
def p3(pressed):
    for i in pressed:
        if int(i[0]) < 49:
            pressed_button(str(i[0]),int(i[1]))
def p4(pressed):
    for i in pressed:
        if int(i[0]) < 49:
            pressed_button(str(i[0]),int(i[1]))
def p5(pressed):
    for i in pressed:
        if int(i[0]) < 49:
            pressed_button(str(i[0]),int(i[1]))

def down_arrow(temp):
    #temp = int(temp)
    temp -=1
def select_all(x):
    x=int(x)
    
    for i in pressed:
        unpressed_button(i[0])
        
    pressed.clear()
    
    if x ==1:
        if pressed.count(['1',3]) == 1:
            for i in range (1,count):
                pressed[pressed.index([str(i),3])] = [str(i),2]
                pressed_button(str(i),2)
                
        elif pressed.count(['1',2]) == 1:
            for i in range (1,count):
                pressed[pressed.index([str(i),2])] = [str(i),1]
                pressed_button(str(i),1)
        elif pressed.count(['1',1]) == 1:
            for i in range (1,count):
                unpressed_button(str(i))
                pressed.remove([str(i),1])
        else:
            for i in range (1,count):
                pressed.append([str(i),3])
                pressed_button(str(i),3)
    if x ==2:
        if pressed.count(['17',3]) == 1:
            for i in range (count,2*count-1):
                pressed[pressed.index([str(i),3])] = [str(i),2]
                pressed_button(str(i),2)
                
        elif pressed.count(['17',2]) == 1:
            for i in range (count,2*count-1):
                pressed[pressed.index([str(i),2])] = [str(i),1]
                pressed_button(str(i),1)
        elif pressed.count(['17',1]) == 1:
            for i in range (count,2*count-1):
                unpressed_button(str(i))
                pressed.remove([str(i),1])
        else:
            for i in range (count,2*count-1):
                pressed.append([str(i),3])
                pressed_button(str(i),3)
    
    if x ==3:
        if pressed.count(['34',3]) == 1:
            for i in range (2*count-1,3*count-2):
                pressed[pressed.index([str(i),3])] = [str(i),2]
                pressed_button(str(i),2)
                
        elif pressed.count(['34',2]) == 1:
            for i in range (2*count-1,3*count-2):
                pressed[pressed.index([str(i),2])] = [str(i),1]
                pressed_button(str(i),1)
        elif pressed.count(['34',1]) == 1:
            for i in range (2*count-1,3*count-2):
                unpressed_button(str(i))
                pressed.remove([str(i),1])
        else:
            for i in range (2*count-1,3*count-2):
                pressed.append([str(i),3])
                pressed_button(str(i),3)
def clearnotes():
    for i in pressed:
        if int(i[0]) < 49:
            unpressed_button(i[0])
    pressed.clear()

#if button is pressed and is off
def pressed_button(x,dyn):
    if dyn ==3:
        window[x].update(button_color='green')
    if dyn ==2:
        window[x].update(button_color='yellow')
    if dyn ==1:
        window[x].update(button_color='red')
#if button is pressed and is on     
def unpressed_button(y):

    window[y].update(button_color='gray')
    
img = sg.Image(filename="/home/pi/Downloads/djem.png")

Top_Text = [img],[sg.Text(text="Djembot",font = 15,pad=10), sg.Text("File Name:", font = 15,pad=10), sg.InputText("Default", key="file_name", font = 15,pad=10), sg.Button('Anthony', key = 'p1',font=15),sg.Button('Kristina', key='p5',font=15), sg.Button('Murray', key = 'p2',font=15),sg.Button('Marcel', font=15,key = 'p3'),sg.Button('Kalani', font=15,key = 'p4')]
sg.theme('DarkRed')


global count
count = 17
input_row1 = [
    [sg.Button(num,button_color = 'black on gray', key = str(num),expand_x = True, expand_y = True, enable_events=True, font = 45,size=(7,4)) for num in range (1,count)]
    ]

input_row2 = [
    [sg.Button(num,button_color = 'black on gray',key = str(num+count-1), enable_events=True, expand_x = True, expand_y = True, font=15,size = (7,4)) for num in range (1,count)]
    ]
input_row3 = [
    [sg.Button(num,button_color = 'black on gray',key = str(num+(count+count)-2), enable_events=True, expand_x = True, expand_y = True,font = 15,size=(7,4)) for num in range (1,count)]
    ]
submit_button = [sg.Button('Save', button_color = 'blue', enable_events=True, size=(7,2),font = 15,pad=(10)),sg.Button("Submit",button_color = 'blue', enable_events=True,font = 15,size=(7,2),pad=(10))]
input_rows= [[sg.Button("Bass",enable_events=True,key='Bass',font=15, size = (7,4),pad=10),sg.Column(input_row1)],[sg.Button("Tone", enable_events=True,font=15,size=(7,4),pad=10),sg.Column(input_row2)],[sg.Button("Slap", enable_events=True,font=15,size=(7,4),pad=10),sg.Column(input_row3)]]
clear_all = sg.Button('Clear', button_color = 'white on red', enable_events = True, font=15,size=(7,2),pad=10,key = 'clearall')
right_column_group = [input_rows,clear_all,submit_button]
Inputs = [[sg.Text("Upload File",font=20,pad=10),sg.Input(key='midifilein',change_submits=True,font=20), sg.FileBrowse(font=20,pad=10), sg.Button('Convert', enable_events=True, font=15)],[sg.Text("Tempo:",font=20,pad=10), sg.Spin(list(range(1,121)),initial_value=60,key="tempo_input",pad=10, size=5,font=20)]]
Loop_in = [[sg.Text("Loops: ",font=20,pad=10)],[sg.InputText(key='loops_in',enable_events=True,size=10,font=20,pad=10)],[sg.Button(button_text="Loop",button_color='black on gray',enable_events=True,font=15,pad=10)]]

layout = [Top_Text,Inputs,Loop_in, right_column_group]

# Create the window
window = sg.Window("Djembot",  layout, resizable=True, finalize = True,auto_size_text=True,auto_size_buttons = True)
pressed = [];
loop = 0;

# Create an event loop
while True:
    event, values = window.read()
    
    if event == "Loop" and loop ==0:
        loop = 1
        pressed_button(event,2)
    elif event == 'Loop' and loop ==1:
        loop = 0
        unpressed_button(event)
    if event in [str(i) for i in range(1, count*3+1)]:      
        if pressed.count([str(event),3])==1:
            print('2')
            pressed[pressed.index([str(event),3])] = [event,2]
            pressed_button(event,2)

        elif pressed.count([str(event),2])==1:
            print('1')
            pressed[pressed.index([str(event),2])] = [event,1]
            pressed_button(event,1)

        elif pressed.count([str(event),1])==1: 
            unpressed_button(event)
            pressed.remove([str(event),1])
            print(pressed)
        else:
            if len(pressed) > 0:
                if int(event) < count and pressed.count([str(int(event)+count-1),3])==0 and pressed.count([str(int(event)+(2*count-2)),3])==0 and pressed.count([str(int(event)+count-1),2])==0 and pressed.count([str(int(event)+(2*count-2)),2])==0 and pressed.count([str(int(event)+count-1),1])==0 and pressed.count([str(int(event)+(2*count-2)),1])==0:
                    pressed_button(event,3)
                    pressed.append([event,3])
                    print (pressed);
                elif int(event) < (count*2)-1 and int(event) > count-1 and pressed.count([str(int(event)-count+1),3])==0 and pressed.count([str(int(event)+(count-1)),3])==0 and pressed.count([str(int(event)-count+1),2])==0 and pressed.count([str(int(event)+(count-1)),2])==0 and pressed.count([str(int(event)-count+1),1])==0 and pressed.count([str(int(event)+(count-1)),1])==0:
                    pressed_button(event,3)
                    pressed.append([event,3])
                    print (pressed);
                elif int(event) > count*2-2 and int(event)<count*3-2 and pressed.count([str(int(event)-2*count+2),3])==0 and pressed.count([str(int(event)-(count)+1),3])==0 and pressed.count([str(int(event)-2*count+2),2])==0 and pressed.count([str(int(event)-(count)+1),2])==0 and pressed.count([str(int(event)-2*count+2),1])==0 and pressed.count([str(int(event)-(count)+1),1])==0:
                    pressed_button(event,3)
                    pressed.append([event,3])
                    print (pressed);
            elif len(pressed) == 0:
                pressed_button(event,3)
                pressed.append([event,3])
                print (pressed);
    if event == "Submit":
        name = str(values['file_name'])
        submit_file(pressed,count,name,values['tempo_input'],loop, values['loops_in'] or 1000, event)
    if event == 'Convert':
        upload_file(values['midifilein'])
    if event == 'Save':
        name = str(values['file_name'])
        submit_file2(pressed,count,name,values['tempo_input'])

    if event == "Stop":
        ser.write(str.encode(str(0) + str(0) + str(0)))
        break
    if event == "Bass":
        select_all(1)
    if event == "Tone":
        select_all(2)
    if event == "Slap":
        select_all(3)
    if event == 'clearall':
        clearnotes()
    if event== 'p1':
        pressed.clear()
        pressed = [['1', 3], ['2', 2], ['3', 1], ['21', 3], ['22', 2], ['23', 1], ['41', 3], ['42', 2], ['43', 1], ['13', 3], ['30', 2], ['47', 1]]
        p1(pressed)
    if event == 'p2':
        pressed.clear()
        pressed = [['1', 1], ['5', 1], ['9', 1], ['13', 1], ['36', 2], ['39', 2], ['47', 2], ['26', 3], ['28', 3]]
        p2(pressed)
    if event == 'p3':
        pressed.clear()
        pressed = [['1', 3], ['18', 2], ['3', 3], ['5', 3], ['22', 2], ['7', 3], ['9', 3], ['26', 2], ['11', 3], ['13', 3], ['30', 2], ['15', 3], ['36', 3], ['40', 3], ['44', 3], ['48', 3]]
        p3(pressed)
    if event == 'p4':
        pressed.clear()
        pressed = [['17', 3], ['18', 2], ['22', 3], ['23', 2], ['25', 3], ['26', 2], ['4', 1], ['8', 1], ['11', 3], ['44', 3], ['45', 2], ['46', 3], ['15', 3], ['48', 3]]
        p4(pressed)
    if event == 'p5':
        pressed.clear()
        pressed = [['1', 3], ['18', 3], ['3', 2], ['20', 2], ['5', 1], ['22', 1], ['7', 3], ['24', 3], ['9', 2], ['26', 2], ['11', 1], ['28', 1], ['13', 3], ['30', 3], ['15', 2], ['32', 3]]
        p5(pressed)
   # if event == "Update":
    #    update_file(values['step_input'])


    if event == sg.WIN_CLOSED:
        break

window.close()
ser.write(str.encode(str(0) + str(0) + str(0)))

